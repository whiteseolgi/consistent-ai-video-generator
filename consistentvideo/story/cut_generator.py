...
class CutGenerator:
    ...
    def cut_scene(self, scene: Dict, entity_list: list, story_text: str) -> List[Dict]:
        prompt = f"""
다음 씬 정보를 컷 단위로 나누어 주세요. 각 컷은 반드시 아래 형식으로 출력해주세요. 하나의 씬당 컷은 4~5개 이하로 해주세요. description에는 컷 내용으로 이미지를 생성할 수 있는 프롬프트를 자세하게 영어로 묘사해주세요.
